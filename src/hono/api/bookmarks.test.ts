...
describe("GET /api/app/bookmarks", () => {
	...
	it("returns the logged-in user's saved posts enriched with commentCount", async () => {
		...
		const json = (await res.json()) as {
			data: { id: string; commentCount: number; videos: unknown[] }[];
		...
		// Wideo płyną teraz z wiersza posta (#194) — samplePost ma pustą listę.
		expect(json.data[0]?.videos).toEqual([]);
	});
